...
#ifndef ssize_t
typedef long long int ssize_t; // 与 corecrt.h 保持一致
#endif
...
inline void AGP::SetForce(int16_t force) {
    if (!is_connected())
        return;
    // 1. 将 force 转换为 uint16_t（避免符号扩展问题）
    uint16_t raw_value = static_cast<uint16_t>(force);

    // 2. 执行高低字节交换
    // uint16_t swapped_value = ((raw_value & 0xFF) << 8) | ((raw_value >> 8) & 0xFF);
...
inline int AGP::modbus_write(uint16_t address, uint16_t amount, int func,
    ...
    if (func == WRITE_COIL || func == WRITE_REG) {
        ...
        for (int i = 0; i < amount; i++) {
            to_send[13 + 2 * i] = (uint8_t)(value[i] >> 8u); //高字节
            to_send[14 + 2 * i] = (uint8_t)(value[i] & 0x00FFu);//低字节

            // to_send[13 + 2 * i] = (uint8_t)(value[i] & 0x00FFu);  // 低字节
            // to_send[14 + 2 * i] = (uint8_t)(value[i] >> 8u);      // 高字节
        }
...
inline int AGP::modbus_read_input_registers(uint16_t address, uint16_t amount,
    ...
    if (_connected) {
        ...
        if (err)
            return err_no;
        for (auto i = 0; i < amount; i++) {
            buffer[i] = ((uint16_t)to_rec[9u + 2u * i]) << 8u;//读写高低位互换 只适用滚边设备
            buffer[i] += (uint16_t)to_rec[10u + 2u * i];
        }
        return 0;
    } else {
